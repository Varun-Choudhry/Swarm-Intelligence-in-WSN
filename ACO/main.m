...
dataset.nodeNo = 20; %Number of nodes
...
dataset.maxEnergy = 100; % Mw - Miliwatts (Full ?. mknjbhvgxe  nergy (100%) - 1 mAh charge capacity within 1 Volt energy)
...
packet=0;
iterationcounter=1;
plotgraphs=1; %1 for "yes" or 0 for "no"
reprodutibily = 0; 
...
name=[strcat('Test-simulation-',num2str(dataset.nodeNo),'nodes-',datestr(now,'mm-dd-yyyy-HH-MM'),'.txt')];
fileID = fopen(fullfile(pwd,name),'w');
fprintf(fileID,'%6s %20s %20s %20s %20s\r\n','|Node No|','|Test Scene|','|Hops|','|Packets sent|','|Dead node|');
...
while ~isempty(G2.Edges)
    ...
    figname=[strcat('FIG-Test-simulation-',num2str(dataset.nodeNo),'nodes-','scene-',num2str(iterationcounter),'-date-',datestr(now,'mm-dd-yyyy-HH-MM'),'.png')];
    saveas(gcf,figname)
